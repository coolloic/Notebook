...
- **[GraphQL Deep-Dive](graphql.html)** — one typed endpoint, client-specified queries: the schema &amp; type system, resolvers, the N+1 problem &amp; DataLoader, cursor pagination, error handling, caching, the security guardrails a public API needs, and federation, with a worked schema design.
- **[Next.js Best Practices](nextjs-best-practices.html)** — the App Router done right: Server vs Client Components, data fetching &amp; the four caching layers, rendering strategies (static, dynamic, streaming, PPR), Server Actions, performance (`next/image`, `next/font`), project structure, and the pitfalls to avoid.
...
- **[Web · API · AI Security](web-ai-security.html)** — a defensive guideline across the surface: HTTP/TLS, authentication &amp; authorization, OWASP web &amp; API risks, AI/agent threats (prompt injection, excessive agency), and a risk program (breach, user, automation).
- **[OWASP — Top 10 &amp; Toolbox](owasp.html)** — a guide to OWASP: what it is, the three Top 10 lists (Web, API, LLM) detailed, and the wider toolbox (ASVS, WSTG, Cheat Sheets, ZAP, Dependency-Check, SAMM, Juice Shop) across the SDLC.
- **[Cyber Security](cyber-security.html)** — the big picture: the CIA triad, security mechanisms &amp; controls, the threat landscape and cyber kill chain, attack prevention (defense in depth, zero trust, MFA), and managing a program (NIST CSF, risk, incident response).
...
- **[Concurrency Design](concurrency-design.html)** — balancing security, idempotency, latency &amp; capacity: architecture options and trade-offs, sizing (Little's Law), load-testing to the knee, retries for isolated failures, and AI-agent context isolation.
- **[PostgreSQL Query Performance](postgres-performance.html)** — how the cost-based planner works, reading `EXPLAIN ANALYZE`, indexes &amp; statistics, common slow-query anti-patterns and their fixes, the tools to find them, and a worked 4.2&nbsp;s → 0.3&nbsp;ms optimisation.
- **[PostgreSQL VACUUM](postgres-vacuum.html)** — why MVCC leaves dead tuples behind, what VACUUM actually reclaims, `VACUUM` vs `VACUUM FULL`, tuning autovacuum (the scale-factor trap), transaction-ID wraparound &amp; freezing, detecting bloat, and a worked queue-table cleanup.
- **[PostgreSQL Indexing Strategies](postgres-indexing.html)** — how a B-tree works, the six index types and when to use each, multicolumn column order (equality-first), covering &amp; partial &amp; expression indexes, building safely with `CONCURRENTLY`, auditing unused/redundant indexes, and a worked minimal-index-set design.
...
- **[GCP Services &amp; Infrastructure as Code](gcp-services.html)** — a field guide to Google Cloud (resource hierarchy, core services by category, a GCP↔AWS map) plus an introduction to Infrastructure as Code with a Terraform-on-GCP walkthrough and best practices.
- **[Terraform vs Pulumi vs OpenTofu](terraform-pulumi-opentofu.html)** — the IaC model all three share, what each tool is, the 2023 BSL licence change that forked OpenTofu from Terraform, the same stack written three ways, and how to choose.
...
- **[AutoGen vs CrewAI](autogen-crewai.html)** — the two leading multi-agent frameworks compared: conversable agents vs role-based crews, their orchestration models, the same task coded both ways, a head-to-head table, and how to characterise the work to choose.
